...
**8. Output Conditions:**
* **If there are no staged files:** Output **ONLY** ask user to stage files first and then retrigger the command again. Never stage file yourself for the commit.
* **If NO breaking change is suspected:** Review the staged changes and proceed with execution of the final executable `git commit -m "..." -m "..."` command. Do not output any extra conversational text or explanations.
